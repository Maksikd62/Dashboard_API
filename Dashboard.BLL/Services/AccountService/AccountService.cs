...
namespace Dashboard.BLL.Services.AccountService
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<ServiceResponse> ConfirmEmail(string userId, string t)
        {
            string token = Encoding.UTF8.GetString(WebEncoders.Base64UrlDecode(t));
            var user = await _userManager.FindByIdAsync(userId);
            var result = await _userManager.ConfirmEmailAsync(user, token);
            if (result.Succeeded)
            {
                return ServiceResponse.GetOkResponse("Підтверджено", true);

            }
            else
            {
                return ServiceResponse.GetInternalServerErrorResponse(message: "Помилка");
            }
        }
    }
}
